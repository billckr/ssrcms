...
**Common/weak-password denylist (2026-09-08):** `validate_password` also rejects any password matching `utils::common_passwords::is_common_password` — a small, hand-curated (not a real breach corpus) list of ~150 long-but-trivially-guessable passwords: a weak base word padded with digits to clear the 12-character floor (`password123456`, `welcome1234567`, ...), keyboard walks, repeated characters, and a few famous passphrases people use ironically/literally (`correcthorsebatterystaple`). Exact-match only (case-insensitive), never a substring check — a substring match on something like "password" would also reject a legitimate long passphrase that merely contains that word, a worse trade than the coverage it buys. Applies everywhere `validate_password` is already called: registration, self-service password changes (account and admin), password recovery, and admin-created/edited users.
