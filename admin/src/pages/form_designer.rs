...
pub fn forms_list_fragment(rows: &[FormRow], page: i64, total_pages: i64, search: &str, sort: &str, dir: &str) -> String {
    ...
    let body = if rows.is_empty() {
        r#"<tr><td colspan="4" style="text-align:center;color:var(--muted)">No forms yet. Create one to get started. <a href="/admin/form-designer/new">New Form</a></td></tr>"#.to_string()
    } else {
        rows.iter().map(|f| {
            format!(
                r#"<tr>
