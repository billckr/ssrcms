...
pub async fn view(
    ...
    let options: Vec<ResultOption> = poll
        .options
        .iter()
        .map(|o| {
            let votes = tally
    ...
    let count = poll_vote::count_for_poll(&state.db, site_id, id)
        .await
        .unwrap_or(0);
    let total_pages = ((count + VOTES_PER_PAGE - 1) / VOTES_PER_PAGE).max(1);
    let offset = (page - 1) * VOTES_PER_PAGE;
    let raw_votes = poll_vote::list_votes(&state.db, site_id, id, VOTES_PER_PAGE, offset)
        .await
        .unwrap_or_default();
    let vote_rows: Vec<VoteRow> = raw_votes
        .iter()
        .map(|v| {
            let option_label = poll
                .options
                .iter()
                .find(|o| o.key == v.option_key)
                .map(|o| o.label.clone())
                .unwrap_or_else(|| v.option_key.clone());
            VoteRow {
                id: v.id.to_string(),
                option_label,
...
pub async fn export_csv(
    ...
    let votes = poll_vote::list_all_votes(&state.db, site_id, id)
        .await
        .unwrap_or_default();
    ...
    (
        [
            ...
            (
                ...
                format!("attachment; filename=\"{}-votes.csv\"", poll.slug),
            ),
        ],
        csv,
    )
