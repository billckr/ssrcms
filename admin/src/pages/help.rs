...
fn render_nav_group(label: &str, entries: &[&HelpEntry]) -> String {
    if entries.is_empty() {
        return String::new();
    }
...
fn render_help_section(e: &HelpEntry) -> String {
    format!(
        r##"<div class="card-boxed doc-section" id="doc-{slug}" style="margin-bottom:2rem">
  <h2 class="card-boxed-header">{title}</h2>
...
pub fn render_list(entries: &[HelpEntry], flash: Option<&str>, ctx: &PageContext) -> String {
    if entries.is_empty() {
        let content = r#"<div class="card" style="padding:2rem;text-align:center;color:var(--muted)">
  <p>No help topics yet.</p>
</div>"#;
        return admin_page("Application Help", "/help", flash, content, ctx);
    }

    let system_entries: Vec<&HelpEntry> = entries.iter().filter(|e| e.grp == "system").collect();
    let feature_entries: Vec<&HelpEntry> = entries.iter().filter(|e| e.grp == "feature").collect();
    let other_entries: Vec<&HelpEntry> = entries
        .iter()
  ...
  {other_nav}
    ...
    let system_sections: String = system_entries.iter().map(|e| render_help_section(e)).collect();
    let feature_sections: String = feature_entries.iter().map(|e| render_help_section(e)).collect();
    let other_sections: String = other_entries.iter().map(|e| render_help_section(e)).collect();
  ...
  <div>{sections}</div>
    ...
    admin_page("Application Help", "/help", flash, &content, ctx)
}
