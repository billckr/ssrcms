...
#   INSTALL_DIR        — where to install (default: /opt/synapcms)
...
ENVIRONMENT VARIABLES
  ...
  INSTALL_DIR          Installation directory                               (default: <current-dir>/synapcms)
...
INSTALL_DIR="${INSTALL_DIR:-${PWD}/synapcms}"
PORT="${PORT:-3000}"
...
chmod +x "${INSTALL_DIR}/synaptic" "${INSTALL_DIR}/synap"
...
if is_rhel_like && command -v chcon &>/dev/null; then
  ...
  chcon -t bin_t "${INSTALL_DIR}/synaptic" "${INSTALL_DIR}/synap" 2>/dev/null || true
...
CLI_OUTPUT=$(sudo -u "${SYNAPTIC_USER}" \
  ...
  ADMIN_EMAIL="$ADMIN_EMAIL" \
  ADMIN_USERNAME="${ADMIN_USERNAME:-}" \
  "${INSTALL_DIR}/synap" install \
  --non-interactive \
  --output-dir "${INSTALL_DIR}" 2>&1) || die "synap install failed:\n$CLI_OUTPUT"
...
if [[ -n "${SKIP_CADDY:-}" ]]; then
  ...
  "${INSTALL_DIR}/synap" caddy setup --app-user "${SYNAPTIC_USER}" \
    && success "Caddy permissions configured." \
    || warn "Caddy permission setup failed — run manually: sudo synap caddy setup --app-user ${SYNAPTIC_USER}"
fi
