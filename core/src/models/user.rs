...
pub struct User {
    ...
    /// Folded into `credential_version()`. Regenerated by
    /// `regenerate_session_nonce` to invalidate every other session
    /// ("sign out other devices") without changing password or email.
...
impl User {
    ...
    /// Opaque marker copied into a session at login. A password change, a
    /// verified email change, or an explicit `regenerate_session_nonce` call
    /// alters this value, invalidating every older session without storing
    /// the password hash itself in session data or scanning the session
    /// table.
...
/// Regenerate a user's session nonce, invalidating every session for this
/// account (including the caller's own) except the one whose caller
/// re-inserts the returned user's fresh `credential_version()` afterward.
pub async fn regenerate_session_nonce(pool: &PgPool, id: Uuid) -> Result<User> {
    let user = sqlx::query_as::<_, User>(
        "UPDATE users SET session_nonce = gen_random_uuid(), updated_at = NOW() WHERE id = $1 RETURNING *",
    )
    .bind(id)
    .fetch_one(pool)
    .await?;
...
mod tests {
    ...
    fn make_user(email: &str, password_hash: &str) -> User {
        make_user_with_nonce(email, password_hash, Uuid::nil())
    }

    fn make_user_with_nonce(email: &str, password_hash: &str, session_nonce: Uuid) -> User {
        User {
            id: Uuid::new_v4(),
    ...
    #[test]
    fn credential_version_changes_when_session_nonce_changes() {
        let a = make_user_with_nonce("same@example.com", "same-hash", Uuid::nil());
        let b = make_user_with_nonce("same@example.com", "same-hash", Uuid::new_v4());
        assert_ne!(a.credential_version(), b.credential_version());
    }
