...
pub fn polls_list_fragment(rows: &[PollRow], search: &str) -> String {
    ...
    let body = if filtered.is_empty() {
        r#"<tr><td colspan="5" style="text-align:center;color:var(--muted)">No polls yet. Create one to get started. <a href="/admin/designer/polls/new">New Poll</a></td></tr>"#.to_string()
    } else {
        filtered.iter().map(|p| {
            format!(
                r#"<tr>
