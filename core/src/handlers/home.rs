...
pub(crate) async fn build_post_context(
    ...
    if let Some(site_id) = p.site_id {
        ctx.content =
            crate::models::form_def::expand_embeds(&state.db, site_id, &ctx.content, None).await;
        ctx.content =
            crate::models::poll_def::expand_embeds(&state.db, site_id, &ctx.content, None).await;
    }
