...
```bash
synap migrate
synap user create   # choose role: super_admin
```
...
```bash
synap install              # Interactive setup wizard
synap migrate              # Apply pending DB migrations
synap user create          # Create a user interactively
synap user list            # List all users
synap user reset-password  # Reset a user's password by email
synap plugin list          # List installed plugins
```

### When to reinstall the CLI

`synap` is a compiled Rust binary installed globally at `~/.cargo/bin/`. It only updates when you explicitly reinstall it — editing source files alone has no effect on the running binary.
...
> **Important:** `cargo build` does NOT update the globally installed `synap`. Only `cargo install --path cli` (or `./app.sh update-cli`) does.

Run `update-cli` whenever you change anything under `cli/src/`. Examples of changes that require it:

| Change | Example |
|---|---|
| Add a new command | Adding `synap backup` |
...
> ```
> Then restart the server. A `synap set-site-url` command (password-gated
