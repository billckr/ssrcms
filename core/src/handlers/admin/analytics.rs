...
pub async fn list(
    ...
    let (rows, flash): (Vec<FormSummaryRow>, Option<&str>) = if tab != "forms" {
        ...
        match form_submission::list_forms(&state.db, site_id).await {
            Ok(summaries) => {
                ...
                let defined: std::collections::HashMap<String, (uuid::Uuid, String)> =
                    form_def::list_for_site(&state.db, site_id)
                        .await
                        .unwrap_or_default()
                        .into_iter()
                        .map(|f| (f.slug, (f.id, f.name)))
                        .collect();
                let mut rows: Vec<FormSummaryRow> = summaries
                    .into_iter()
                    .map(|s| {
                        ...
                        let display_name = def
                            ...
                            .unwrap_or_else(|| s.form_name.clone());
                        FormSummaryRow {
    ...
    Html(admin::pages::analytics::render(
        tab,
        &rows,
        &q.sort,
        &q.dir,
...
pub async fn form_detail(
    ...
    let submissions = if tab == "submissions" {
        ...
        let total: i64 = form_submission::count_for_form(&state.db, site_id, &form.slug)
            .await
            .unwrap_or(0);
        ...
        let all_data = form_submission::list_all_data_for_form(&state.db, site_id, &form.slug)
            .await
            .unwrap_or_default();
        let columns = super::forms::collect_columns(&all_data.iter().collect::<Vec<_>>());

        let rows: Vec<admin::pages::forms::SubmissionRow> =
            form_submission::list_submissions(&state.db, site_id, &form.slug, PER_PAGE, offset)
                .await
                .unwrap_or_default()
                .into_iter()
        ...
        Some(admin::pages::analytics::SubmissionsTabData {
            rows,
            columns,
            page,
            total_pages,
    ...
    let (total_sent, succeeded, failed) = mail_log::counts_for_form(&state.db, id)
        .await
        .unwrap_or((0, 0, 0));
    let recent = mail_log::list_for_form(&state.db, id, 50)
        .await
        .unwrap_or_default();

    let mut recent: Vec<admin::pages::analytics::MailLogRow> = recent
        .into_iter()
        .map(|r| admin::pages::analytics::MailLogRow {
            to_email: r.to_email,
            subject: r.subject,
            success: r.success,
    ...
    if params.contains_key("partial") {
        let search_qs = if search.is_empty() {
            ...
            format!("&search={}", admin::html_escape(search))
        };
        return Html(admin::pages::analytics::render_analytics_table(
            &data, sort, dir, &search_qs,
        ))
        .into_response();
    }

    Html(admin::pages::analytics::render_analytics(
        &data, tab, sort, dir, search, &ctx,
    ))
    .into_response()
}
