...
pub async fn list(
    ...
    let active_type: Option<&str> = params
        .get("type")
        .map(|s| s.as_str())
        .filter(|s| ["image", "video", "audio", "document"].contains(s));

    let uploaded_by = if admin.site_role == Some(crate::models::site_user::SiteRole::Author) {
        Some(admin.user.id)
    } else {
        None
    };
    ...
    let folders = if let Some(sid) = admin.site_id {
        crate::models::media_folder::list(&state.db, sid)
            .await
            .unwrap_or_default()
    } else {
        vec![]
    ...
    let type_counts = {
        ...
        .await
        .unwrap_or_default();
        let mut tc = admin::pages::media::TypeCounts {
            all: 0,
            image: 0,
            video: 0,
            audio: 0,
    ...
    let uploader_ids: Vec<Uuid> = raw
        .iter()
        .map(|m| m.uploaded_by)
        .collect::<std::collections::HashSet<_>>()
        .into_iter()
        .collect();
    let uploader_names: std::collections::HashMap<Uuid, String> =
    ...
    let items: Vec<admin::pages::media::MediaItem> = raw
        .iter()
        .map(|m| {
            ...
            admin::pages::media::MediaItem {
                ...
                uploaded_by_name: uploader_names
                    .get(&m.uploaded_by)
                    .cloned()
    ...
    let folder_items: Vec<admin::pages::media::FolderItem> = folders
        .iter()
        .map(|f| admin::pages::media::FolderItem {
            id: f.id.to_string(),
            name: f.name.clone(),
        })
        .collect();
...
pub async fn api_grid(
    ...
    let folder_items: Vec<GridFolder> = folders
        .iter()
        .map(|f| GridFolder {
            id: f.id.to_string(),
            name: f.name.clone(),
    ...
    match crate::models::media::get_by_id(&state.db, id).await {
        Ok(media) => {
            ...
            if admin.site_role == Some(crate::models::site_user::SiteRole::Author)
                && media.uploaded_by != admin.user.id
            {
                tracing::warn!(
...
pub async fn api_update_folder(
    ...
    let folder_id: Option<Uuid> = body
        .get("folder_id")
        .and_then(|v| v.as_str())
        .filter(|s| !s.is_empty())
        .and_then(|s| s.parse().ok());
    ...
    if let Some(fid) = folder_id {
        if !admin.caps.is_global_admin {
            let folder_site: Option<uuid::Uuid> =
                sqlx::query_scalar("SELECT site_id FROM media_folders WHERE id = $1")
                    .bind(fid)
            ...
            match folder_site {
                None => {
                    return (
                        axum::http::StatusCode::NOT_FOUND,
                        axum::Json(serde_json::json!({"error":"Folder not found"})),
                    )
                        .into_response()
                }
...
pub async fn api_list(State(state): State<AppState>, admin: AdminUser) -> impl IntoResponse {
    #[derive(serde::Serialize)]
    struct Item {
        id: String,
        filename: String,
...
pub async fn delete_folder(
    ...
    let delete_media = body
        .get("delete_media")
        .map(|s| s == "true")
        .unwrap_or(false);
    if let Some(site_id) = admin.site_id {
        let folder_name = crate::models::media_folder::list(&state.db, site_id)
            .await
            .unwrap_or_default()
            .into_iter()
            .find(|f| f.id == id)
        ...
        if delete_media {
            ...
            let items =
                crate::models::media::list(&state.db, Some(site_id), None, Some(id), 10_000, 0)
                    .await
                    .unwrap_or_default();
            for m in &items {
        ...
        if crate::models::media_folder::delete(&state.db, id, site_id)
            .await
            .is_ok()
        {
            super::audit(
