...
pub fn build(
    ...
    let upload_limit = tower::ServiceBuilder::new()
        ...
        .layer(middleware::from_fn_with_state(
            state.clone(),
            crate::middleware::upload_limit::gate,
        ));
    ...
    let maintenance_layer =
        middleware::from_fn_with_state(state.clone(), crate::middleware::maintenance::gate);
    let ip_allowlist_layer =
        middleware::from_fn_with_state(state.clone(), crate::middleware::ip_allowlist::gate);
    let ip_denylist_layer =
        middleware::from_fn_with_state(state.clone(), crate::middleware::ip_denylist::gate);
    ...
    let admin_router = Router::new()
        ...
        .route(
            ...
            post(dashboard::save_widget_layout),
        )
        .route(
            "/admin/dashboard/dismiss-welcome",
            post(dashboard::dismiss_welcome_panel),
        )
        // ── Admin posts ────────────────────────────────────────────────────
        .route("/admin/posts", get(posts::list))
        .route(
            "/admin/posts/new",
            get(posts::new_post).post(posts::save_new),
        )
        .route(
            "/admin/posts/{id}/edit",
        ...
        .route(
            "/admin/api/posts/{id}/sources",
            post(posts::api_set_sources),
        )
        ...
        .route(
            "/admin/pages/new",
            get(posts::new_page).post(posts::save_new),
        )
        .route(
            "/admin/pages/{id}/edit",
            get(posts::edit_page).post(posts::save_edit),
        )
        ...
        .route(
            "/admin/api/media/{id}/folder",
            post(media::api_update_folder),
        )
        // ── Admin media ────────────────────────────────────────────────────
        .route("/admin/media", get(media::list))
        .route(
            "/admin/media/upload",
            post(upload::upload).layer(upload_limit.clone()),
        )
        .route("/admin/media/folders/new", post(media::create_folder))
        .route(
            "/admin/media/folders/{id}/delete",
            post(media::delete_folder),
        )
        ...
        .route(
            "/admin/users/new",
            get(users::new_user).post(users::save_new),
        )
        .route(
            "/admin/users/{id}/edit",
            get(users::edit_user).post(users::save_edit),
        )
        ...
        .route(
            "/admin/users/{id}/site-access",
            get(users::site_access_page),
        )
        .route(
            "/admin/users/{id}/site-access/add",
            post(users::add_site_access),
        )
        .route(
            "/admin/users/{id}/site-access/remove",
            post(users::remove_site_access),
        )
        ...
        .route(
            "/admin/themes/publish-theme",
            post(themes_publish::publish_theme),
        )
        .route("/admin/themes/delete", post(themes::delete))
        .route(
            "/admin/themes/upload",
            post(themes_upload::upload_theme).layer(upload_limit.clone()),
        )
        .route(
            "/admin/theme-screenshot/{theme_name}",
            get(themes::screenshot),
        )
        .route(
            "/admin/themes/create",
            get(themes_upload::create_form).post(themes_upload::create_theme),
        )
        .route(
            "/admin/themes/editor/{theme}",
            get(themes_editor::edit_file),
        )
        .route(
            "/admin/themes/editor/{theme}/save",
            post(themes_editor::save_file),
        )
        .route(
            "/admin/themes/editor/{theme}/restore",
            post(themes_editor::restore_file),
        )
        .route(
            "/admin/themes/editor/{theme}/new-file",
            post(themes_editor::new_file),
        )
        .route(
            "/admin/themes/editor/{theme}/delete-file",
            post(themes_editor::delete_file),
        )
        .route(
            "/admin/themes/editor/{theme}/customizer-save",
            post(themes_editor::save_customizer),
        )
        .route(
            "/admin/themes/editor/{theme}/customizer-reset",
            post(themes_editor::reset_options),
        )
        // Old URL, kept as a permanent redirect for existing bookmarks/links.
        .route(
            "/admin/appearance",
            get(|| async { axum::response::Redirect::permanent("/admin/themes") }),
        )
        // ── Page builder ───────────────────────────────────────────────────
        ...
        .route(
            ...
            post(admin_builder::deactivate_project),
        )
        ...
        .route(
            "/admin/builder/{project_id}",
            get(admin_builder::project_pages),
        )
        .route(
            "/admin/builder/{project_id}/rename",
            post(admin_builder::rename_project),
        )
        .route(
            "/admin/builder/{project_id}/activate",
            post(admin_builder::activate_project),
        )
        .route(
            "/admin/builder/{project_id}/delete",
            post(admin_builder::delete_project),
        )
        .route(
            "/admin/builder/{project_id}/pages/new",
            get(admin_builder::new_page_form).post(admin_builder::create_page),
        )
        .route(
            "/admin/builder/{project_id}/pages/{page_id}",
            get(admin_builder::edit_page),
        )
        .route(
            "/admin/builder/{project_id}/pages/{page_id}/set-homepage",
            post(admin_builder::set_homepage),
        )
        .route(
            "/admin/builder/{project_id}/pages/{page_id}/duplicate",
            post(admin_builder::duplicate_page),
        )
        .route(
            "/admin/builder/{project_id}/pages/{page_id}/delete",
            post(admin_builder::delete_page),
        )
        // ── Admin menus ────────────────────────────────────────────────────
        .route(
            "/admin/menus",
            get(admin_menus::list).post(admin_menus::create),
        )
        .route(
            "/admin/menus/{id}",
            get(admin_menus::edit).post(admin_menus::update),
        )
        ...
        .route(
            ...
            post(admin_menus::edit_item),
        )
        .route(
            "/admin/menus/{id}/items/{item_id}/delete",
            post(admin_menus::delete_item),
        )
        .route(
            "/admin/menus/{id}/items/reorder",
            post(admin_menus::reorder_items),
        )
        // ── Admin settings ─────────────────────────────────────────────────
        .route(
            "/admin/settings",
            get(settings::settings).post(settings::save_settings),
        )
        .route(
            "/admin/site-settings",
            get(site_settings::view).post(site_settings::save_general),
        )
        .route(
            "/admin/site-settings/logo",
            post(site_settings::upload_logo),
        )
        .route(
            "/admin/site-settings/logo/reset",
            post(site_settings::reset_logo),
        )
        .route("/admin/settings/logo", post(logo_upload::upload_logo))
        .route("/admin/settings/logo/reset", post(logo_upload::reset_logo))
        .route(
            "/admin/settings/dev-tools/seed-users",
            post(dev_tools::seed_users),
        )
        .route(
            "/admin/settings/dev-tools/seed-posts",
            post(dev_tools::seed_posts),
        )
        .route(
            "/admin/settings/dev-tools/clear",
            post(dev_tools::clear_test_data),
        )
        .route(
            "/admin/settings/dev-tools/nuke-all",
            post(dev_tools::nuke_all),
        )
        .route(
            "/admin/settings/dev-tools/reindex-search",
            post(dev_tools::reindex_search),
        )
        // ── Admin sites ────────────────────────────────────────────────────
        .route(
            "/admin/pick-role",
            get(role_picker::show).post(role_picker::submit),
        )
        .route(
            "/admin/sites",
            get(admin_sites::list).post(admin_sites::create),
        )
        ...
        .route(
            ...
            get(admin_sites::site_settings),
        )
        .route(
            "/admin/sites/{id}/site-config",
            post(admin_sites::save_site_config),
        )
        .route(
            "/admin/sites/{id}/maintenance",
            post(admin_sites::save_maintenance),
        )
        .route(
            "/admin/sites/{id}/import-wp",
            post(wp_import::import).layer(upload_limit.clone()),
        )
        .route("/admin/sites/{id}/import-wp/status", get(wp_import::status))
        .route(
            "/admin/sites/{id}/import-wp/credentials.csv",
            get(wp_import::credentials_csv),
        )
        .route(
            "/admin/sites/{id}/email-providers",
            post(admin_email_providers::create),
        )
        .route(
            "/admin/sites/{id}/email-providers/{provider_id}",
            post(admin_email_providers::update),
        )
        .route(
            "/admin/sites/{id}/email-providers/{provider_id}/test",
            post(admin_email_providers::test),
        )
        .route(
        ...
        .route(
            "/admin/sites/{id}/provision-ssl",
            post(admin_sites::provision_ssl),
        )
        // ── Admin forms / polls ──────────────────────────────────────────────
        .route("/admin/designer", get(admin_designer_hub::list))
        .route(
            "/admin/form-designer",
            get(admin_form_designer::list).post(admin_form_designer::create),
        )
        .route(
            "/admin/form-designer/new",
            get(admin_form_designer::new_form),
        )
        .route(
            "/admin/form-designer/{id}",
            get(admin_form_designer::edit_form).post(admin_form_designer::update),
        )
        .route(
            "/admin/form-designer/{id}/delete",
        ...
        .route(
            "/admin/designer/polls/new",
            get(admin_poll_designer::new_poll),
        )
        .route(
            "/admin/designer/polls/{id}",
            get(admin_poll_designer::edit_poll).post(admin_poll_designer::update),
        )
        .route(
        ...
        .route(
            ...
            get(admin_poll_results::view),
        )
        .route(
            "/admin/designer/polls/{id}/results/export",
            get(admin_poll_results::export_csv),
        )
        .route(
            "/admin/designer/polls/{id}/results/reset",
            post(admin_poll_results::reset),
        )
        .route("/admin/analytics", get(admin_analytics::list))
        .route(
            "/admin/analytics/form/{id}",
            get(admin_analytics::form_detail),
        )
        .route(
            "/admin/form-data-analytics/{name}",
            get(admin_forms::view_form),
        )
        .route(
            "/admin/form-data-analytics/{name}/{id}/delete",
            post(admin_forms::delete_submission),
        )
        .route(
            "/admin/form-data-analytics/{name}/delete-all",
            post(admin_forms::delete_all),
        )
        .route(
            "/admin/form-data-analytics/{name}/export",
            get(admin_forms::export_csv),
        )
        .route(
            "/admin/form-data-analytics/{name}/toggle-block",
            post(admin_forms::toggle_block),
        )
        // ── Static files ───────────────────────────────────────────────────
