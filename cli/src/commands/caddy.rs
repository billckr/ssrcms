...
fn setup(app_user: &str, caddyfile_path: &str) -> anyhow::Result<()> {
    ...
    // 2. Make the Caddyfile group-writable by the caddy group so the app user
    //    can append/remove blocks. Must also force the file's *group owner*
    ...
    let status = Command::new("chgrp")
        ...
        .status()
        .map_err(|e| anyhow::anyhow!("Failed to run chgrp: {e}"))?;
    if !status.success() {
        anyhow::bail!(
