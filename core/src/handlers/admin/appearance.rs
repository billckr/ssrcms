...
pub async fn list(
    ...
    render_appearance_list(&state, None, &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin").await
}
...
pub async fn activate(
    ...
    if form.theme.contains("..") || form.theme.contains('/') || form.theme.contains('\\') {
        return render_appearance_list(&state, Some("Invalid theme name."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin").await.into_response();
    }
    ...
    let theme_path = if global_dir.join(&form.theme).is_dir() {
        ...
        if sd.join(&form.theme).is_dir() {
            ...
            return render_appearance_list(&state, Some("Theme not found."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin").await.into_response();
        }
    } else {
    ...
    let canonical_theme = match theme_path.canonicalize() {
        Ok(p) => p,
        Err(_) => return render_appearance_list(&state, Some("Theme not found."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin").await.into_response(),
    };
    ...
    let site_id = match admin.site_id {
        ...
        None => {
            ...
            return render_appearance_list(&state, Some("No site selected. Run 'synaptic-cli site init' first."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin").await.into_response();
        }
    };
    ...
    if let Err(e) = set_site_setting(&state.db, site_id, "active_theme", &form.theme).await {
        ...
        return render_appearance_list(&state, Some("Failed to activate theme. Please try again."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin").await.into_response();
    }

    if let Err(e) = state.templates.switch_theme(&form.theme) {
        tracing::error!("failed to switch theme to '{}': {:?}", form.theme, e);
        return render_appearance_list(&state, Some("Theme files could not be loaded. Please try again."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin").await.into_response();
    }
...
pub async fn delete(
    ...
    macro_rules! err {
        ($msg:expr) => {
            return render_appearance_list(&state, Some($msg), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin")
                .await
                .into_response()
        };
    }
    ...
    render_appearance_list(&state, Some(&format!("Theme '{}' deleted.", form.theme)), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin")
        .await
        .into_response()
}
...
pub async fn upload_theme(
    ...
    while let Ok(Some(field)) = multipart.next_field().await {
        if field.name().unwrap_or("") == "file" {
            match field.bytes().await {
                ...
                Ok(_) => {
                    return render_appearance_list(&state, Some("Upload too large. Maximum size is 50 MB."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin")
                        .await
                        .into_response();
                }
                Err(e) => {
                    tracing::error!("failed to read theme zip field: {:?}", e);
                    return render_appearance_list(&state, Some("Failed to read uploaded file. Please try again."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin")
                        .await
                        .into_response();
                }
            }
    ...
    let zip_bytes = match zip_bytes {
        Some(b) => b,
        None => return render_appearance_list(&state, Some("No file received."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin").await.into_response(),
    };
    ...
    let target_dir = if admin.is_global_admin {
        ...
        return render_appearance_list(&state, Some("No site selected. Cannot install theme."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin")
            .await
            .into_response();
    };
    ...
    if let Err(e) = std::fs::create_dir_all(&target_dir) {
        ...
        return render_appearance_list(&state, Some("Failed to prepare theme directory."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin")
            .await
            .into_response();
    }
    ...
    match result {
        Ok(Ok(theme_name)) => {
            ...
            if let Err(e) = state.templates.switch_theme(&active) {
                ...
                return render_appearance_list(&state, Some("Theme installed but could not be reloaded. Please restart the server."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin")
                    .await
                    .into_response();
            }
            tracing::info!("reloaded active theme '{}' after installing '{}'", active, theme_name);

            render_appearance_list(&state, Some(&format!("Theme '{}' installed successfully.", theme_name)), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin")
                .await
                .into_response()
        }
        Ok(Err(msg)) => {
            tracing::warn!("theme upload rejected: {}", msg);
            render_appearance_list(&state, Some(&msg), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin").await.into_response()
        }
        Err(e) => {
            tracing::error!("theme upload task panicked: {:?}", e);
            render_appearance_list(&state, Some("Installation failed. Please try again."), &cs, admin.is_global_admin, admin.is_visiting_foreign_site, admin.site_id, &admin.user.email, admin.is_global_admin || admin.site_role.as_str() == "admin")
                .await
                .into_response()
        }
    }
...
async fn render_appearance_list(
    ...
    Html(render_with_flash(&themes, flash, current_site, is_global_admin, visiting_foreign_site, user_email, can_manage_users))
}
